...
void PricerDBIB::project(Policy& policy,  std::map<int, std::vector<std::vector<double>>> &indexScenario, std::vector<double> &fw, int sceInd, int timeInd) {
	//time to maturity
	...
	if (T == 0 or (T > 0 and Param::NUMSTEP -1 == timeInd)) {
		double rg = 1 / getAnnuityFactor_r(policy, 0.05);
		double aT = getAnnuityFactor(policy, timeInd, fw);
		LA[sceInd][timeInd] = std::max(policy.gbAmt * rg * aT - dAV, 0.0);
	}
